...
<div class="container col-md-6">
    <div class="card card-primary">
        <div class="card-header">
            <h3 class="card-title">Gambar Post Header</h3>
        </div>
        <!-- /.card-header -->
        <div class="card-body">
            <div class="my-3">
                <img class="img-fluid img-thumbnail" width="100%" src="{{ $post_header->takeImage }}">
            </div>

            <form action="{{ route('post_header.updateImage', $post_header->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
...
<div class="container">
    <div class="card card-primary">
        <div class="card-header">
            <h3 class="card-title">Data Post Header</h3>
        </div>
        <!-- /.card-header -->
        <div class="card-body">
    ...
    <div class="modal-dialog modal-md">
        <div class="modal-content">
            ...
            <form method="post" id="itemForm" name="itemForm">
                @csrf
                <input type="hidden" name="post_header_id" id="post_header_id">
...
$(document).ready(function () {
    ...
    $('body').on('click', '#editItem', function () {
        var post_header_id = $(this).data('id');
        $.get("{{ route('post_header.index') }}" +'/' + post_header_id +'/edit', function (data) {
            $('#modal-md').modal('show');
            setTimeout(function () {
                $('#title').focus();
            }, 500);
